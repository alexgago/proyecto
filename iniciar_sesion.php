...
<body>
	...
	if ($_SERVER['REQUEST_METHOD'] != 'GET' && empty($errores)) {
		...
		$usuarios = $con->seleccionarUsuarios();
		if (isset($_POST['enviar'])) {
			for ($i = 0; $i < count($usuarios); $i++) {
				if ($email == $usuarios[$i]["correo"] && $dni == $usuarios[$i]["DNI"]) {
	...
	?>
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
			<p>Iniciar sesion</p>
			<label for="email" style="width: 200px;">EMAIL:</label><br>
			<input type="text" style="width: 400px;" name="email"><br><br>
			<label for="DNI" style="width: 200px;">DNI:</label><br>
			<input type="text" style="width: 400px;" name="dni"><br><br>
